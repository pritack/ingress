// ==UserScript==
// @id             iitc-plugin-player-tracker-6h@breunigs
// @name           IITC Plugin: Player tracker 6hour
// @category       Layer
// @version        0.1
...
// @description    [iitc-2017-02-06-004740] Draw trails for the path a user took onto the map based on status messages in COMMs. Uses up to three hours of data. Does not request chat data on its own, even if that would be useful.
...
function wrapper(plugin_info) {
    ...
    plugin_info.dateTimeVersion = '20170206.0000';
    plugin_info.pluginId = 'player-tracker-6hour';
    ...
    window.plugin.playerTracker6h.setup = function() {
        ...
        plugin.playerTracker6h.iconEnl = L.Icon.Default.extend({options: {
            iconUrl: iconEnlImage,
            iconRetinaUrl: iconEnlRetImage
        }});
        plugin.playerTracker6h.iconRes = L.Icon.Default.extend({options: {
            iconUrl: iconResImage,
            iconRetinaUrl: iconResRetImage
        }});

        plugin.playerTracker6h.drawnTracesEnl = new L.LayerGroup();
        plugin.playerTracker6h.drawnTracesRes = new L.LayerGroup();
        // to avoid any favouritism, we'll put the player's own faction layer first
        if (PLAYER.team == 'RESISTANCE') {
            window.addLayerGroup('Player Tracker Resistance 6hour', plugin.playerTracker6h.drawnTracesRes, true);
            window.addLayerGroup('Player Tracker Enlightened 6hour', plugin.playerTracker6h.drawnTracesEnl, true);
        } else {
            window.addLayerGroup('Player Tracker Enlightened 6hour', plugin.playerTracker6h.drawnTracesEnl, true);
            window.addLayerGroup('Player Tracker Resistance 6hour', plugin.playerTracker6h.drawnTracesRes, true);
        }
        map.on('layeradd',function(obj) {
            if(obj.layer === plugin.playerTracker6h.drawnTracesEnl || obj.layer === plugin.playerTracker6h.drawnTracesRes) {
        ...
        plugin.playerTracker6h.playerPopup = new L.Popup({offset: L.point([1,-34])});

        addHook('publicChatDataAvailable', window.plugin.playerTracker6h.handleData);

        window.map.on('zoomend', function() {
            window.plugin.playerTracker6h.zoomListener();
        });
        window.plugin.playerTracker6h.zoomListener();

        plugin.playerTracker6h.setupUserSearch();
    };

    window.plugin.playerTracker6h.stored = {};

    plugin.playerTracker6h.onClickListener = function(event) {
        var marker = event.target;

        if (marker.options.desc) {
            plugin.playerTracker6h.playerPopup.setContent(marker.options.desc);
            plugin.playerTracker6h.playerPopup.setLatLng(marker.getLatLng());
            map.openPopup(plugin.playerTracker6h.playerPopup);
        }
    };

    // force close all open tooltips before markers are cleared
    window.plugin.playerTracker6h.closeIconTooltips = function() {
        plugin.playerTracker6h.drawnTracesRes.eachLayer(function(layer) {
            if ($(layer._icon)) { $(layer._icon).tooltip('close');}
        });
        plugin.playerTracker6h.drawnTracesEnl.eachLayer(function(layer) {
            if ($(layer._icon)) { $(layer._icon).tooltip('close');}
        });
    };
    ...
    window.plugin.playerTracker6h.discardOldData = function() {
        var limit = plugin.playerTracker6h.getLimit();
        $.each(plugin.playerTracker6h.stored, function(plrname, player) {
            var i;
            ...
            if(i === 0) return true;
            if(i === ev.length) return delete plugin.playerTracker6h.stored[plrname];
            plugin.playerTracker6h.stored[plrname].events.splice(0, i);
        });
    };

    window.plugin.playerTracker6h.eventHasLatLng = function(ev, lat, lng) {
        var hasLatLng = false;
    ...
    window.plugin.playerTracker6h.processNewData = function(data) {
        var limit = plugin.playerTracker6h.getLimit();
        $.each(data.result, function(ind, json) {
            ...
            var playerData = window.plugin.playerTracker6h.stored[plrname];

            // short-path if this is a new player
            if(!playerData || playerData.events.length === 0) {
                plugin.playerTracker6h.stored[plrname] = {
                    nick: plrname,
            ...
            if(evts[cmp].time === json[1]) {
                ...
                plugin.playerTracker6h.stored[plrname].events = evts;
                return true;
            }
            ...
            if(evts[cmp+1] && plugin.playerTracker6h.eventHasLatLng(evts[cmp+1], lat, lng))
                return true;

            // if this event is newer, need to look at the previous one
            var sameLocation = plugin.playerTracker6h.eventHasLatLng(evts[cmp], lat, lng);
    ...
    window.plugin.playerTracker6h.getLatLngFromEvent = function(ev) {
    ...
    window.plugin.playerTracker6h.ago = function(time, now) {
        var s = (now-time) / 1000;
    ...
    window.plugin.playerTracker6h.drawData = function() {
        var isTouchDev = window.isTouchDevice();

        var gllfe = plugin.playerTracker6h.getLatLngFromEvent;
        ...
        $.each(plugin.playerTracker6h.stored, function(plrname, playerData) {
            ...
            var ago = plugin.playerTracker6h.ago;
            ...
            if(evtsLength >= 2) {
                ...
                for(var j = evtsLength - 2; j >= 0 && i >= evtsLength - 10; j--) {
                    var ev = playerData.events[j];
                    $('<tr>')
                        .append($('<td>')
                                .text(ago(ev.time, now) + ' ago'))
                        .append($('<td>')
                                .append(plugin.playerTracker6h.getPortalLink(ev)))
                        .appendTo(table);
                }
            }
            ...
            m.addEventListener('spiderfiedclick', plugin.playerTracker6h.onClickListener);
            ...
            m.addTo(playerData.team === 'RESISTANCE' ? plugin.playerTracker6h.drawnTracesRes : plugin.playerTracker6h.drawnTracesEnl);
        ...
        $.each(polyLineByAgeEnl, function(i, polyLine) {
            ...
            $.each(polyLine,function(ind,poly) {
                L.polyline(poly, opts).addTo(plugin.playerTracker6h.drawnTracesEnl);
            });
        });
        $.each(polyLineByAgeRes, function(i, polyLine) {
            ...
            $.each(polyLine, function(ind,poly) {
                L.polyline(poly, opts).addTo(plugin.playerTracker6h.drawnTracesRes);
            });
        });
    };

    window.plugin.playerTracker6h.getPortalLink = function(data) {
    ...
    window.plugin.playerTracker6h.handleData = function(data) {
        ...
        plugin.playerTracker6h.drawnTracesEnl.clearLayers();
        plugin.playerTracker6h.drawnTracesRes.clearLayers();
        plugin.playerTracker6h.drawData();
    };

    window.plugin.playerTracker6h.findUser = function(nick) {
    ...
    window.plugin.playerTracker6h.findUserPosition = function(nick) {
        var data = window.plugin.playerTracker6h.findUser(nick);
        if (!data) return false;

        var last = data.events[data.events.length - 1];
        return plugin.playerTracker6h.getLatLngFromEvent(last);
    };

    window.plugin.playerTracker6h.centerMapOnUser = function(nick) {
        var data = plugin.playerTracker6h.findUser(nick);
        if(!data) return false;

        var last = data.events[data.events.length - 1];
        var position = plugin.playerTracker6h.getLatLngFromEvent(last);

        if(window.isSmartphone()) window.show('map');
        window.map.setView(position, map.getZoom());

        if(data.marker) {
            window.plugin.playerTracker6h.onClickListener({target: data.marker});
        }
        return true;
    };
    ...
    window.plugin.playerTracker6h.onSearchResultSelected = function(result, event) {
        ...
        if(false === window.plugin.playerTracker6h.centerMapOnUser(result.nickname))
    ...
    window.plugin.playerTracker6h.onSearch = function(query) {
        var term = query.term.toLowerCase();

        if (term.length && term[0] == '@') term = term.substr(1);

        $.each(plugin.playerTracker6h.stored, function(nick, data) {
            ...
            query.addResult({
                ...
                position: plugin.playerTracker6h.getLatLngFromEvent(event),
                onSelected: window.plugin.playerTracker6h.onSearchResultSelected,
            });
        });
    };

    window.plugin.playerTracker6h.setupUserSearch = function() {
        addHook('nicknameClicked', window.plugin.playerTracker6h.onNicknameClicked);
        addHook('search', window.plugin.playerTracker6h.onSearch);
    };


    var setup = plugin.playerTracker6h.setup;
